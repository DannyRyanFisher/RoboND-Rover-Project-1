...
class Rover_commands_basic():
    ...
    def apply_brakes_lightly(Rover):
...
class Driving(Rover_commands_basic):
    ...
    def drive_to_specified_speed(self, specified_speed):
        ...
        if self.Rover.vel < specified_speed:
                Driving.release_brakes(self.Rover)
                Driving.drive_forwards_at_custom_throttle(self.Rover, 0.6)
        else: 
                Driving.coast(self.Rover)
                #Driving.apply_brakes_lightly(self.Rover)
...
class Steering():
    ...
    def steer_median_biased_left(self):
        ...
        a = np.clip(self.Rover.nav_angles, -0.5, 0.8)
